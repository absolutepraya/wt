...
- `test/installer.test.ts`: exercises exact-tag downloads, override paths, metadata, preflight and failed-install final-file invariants including a forced later move failure, malicious and valid constrained CDN redirects, redirect limits, API/checksum/missing/malformed failures, startup smoke failure, hostile shell paths, malformed managed blocks, idempotent Bash/Zsh/Fish managed blocks, local-source installation, and the exact missing-dist message.
