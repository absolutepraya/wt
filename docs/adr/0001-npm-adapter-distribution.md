...
We will publish `@absolutepraya/wt` as a public, project-local npm package that exposes the `wt` command through a thin adapter around the existing core CLI. Consumer projects own their lockfiles, while the standalone installer remains responsible for global installation and shell integration. This preserves the existing CLI contract without requiring a Node rewrite, bundled runtime, or install-time shell mutation.
