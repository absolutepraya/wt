...
test("rejects a missing worktree beneath a symlink escaping the root", () => { const root = mkdtempSync(join(tmpdir(), "wt-root-")); const outside = mkdtempSync(join(tmpdir(), "wt-outside-")); symlinkSync(outside, join(root, "link")); assert.throws(() => assertInsideWorktreeRoot(join(root, "link", "new-worktree"), root), ConfigurationError); });
test("rejects an absolute relative result for another Windows volume", () => { if (process.platform === "win32") assert.throws(() => assertInsideWorktreeRoot("D:\\outside\\new", "C:\\repo"), ConfigurationError); });
